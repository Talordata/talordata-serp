...
def test_serp_client_posts_form_encoded_request():
    ...
    assert request.headers["User-agent"] == "Talordata-Dify-Plugin/0.1.13"
    assert request.headers["Accept"] == "application/json"
    assert request.headers["Content-type"] == "application/x-www-form-urlencoded"
    assert request.data == b"engine=bing_images&q=coffee&count=3&json=1&isjson=1"
    assert timeout == 60
...
@pytest.mark.parametrize("mode", [1, 2, 3, 6, 7, 42])
def test_serp_client_preserves_each_explicit_json_value(mode):
    calls = []
    ...
    client = SerpClient(endpoint=ENDPOINT, api_key="sk_test", urlopen=fake_urlopen)
    client.request({"engine": "google", "q": "coffee", "json": mode})

    request, _ = calls[0]
    assert f"json={mode}".encode("ascii") in request.data.split(b"&")
